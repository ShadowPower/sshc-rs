...
在单台服务器上交互式执行远程命令，强制分配 TTY，适合 `vim`、`top`、`less`、远程 REPL 等需要实时按键输入的程序。
...
- `tty` 会把本地终端直接连接到远程 PTY，方向键、Ctrl 组合键、Esc 等按键会原样发送给远端程序。
- 这类命令不会像 `run` 那样汇总输出，也不支持并行执行。
- 没有远程命令时，请直接使用 `sshc connect <服务器名称>` 或 `sshc c <服务器名称>` 进入远程 shell。

### 示例:

`sshc tty prod-server -- vim /etc/nginx/nginx.conf`
`sshc tty prod-server -- top`
`sshc tty sudo prod-server -- htop`
